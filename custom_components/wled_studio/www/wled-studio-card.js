...
`,firmware:t=>e`
  <wled-view-firmware
    ...
    .controllerId=${t.controllerId}
  ></wled-view-firmware>
`};const V=.6,D=.86,R="blur(24px) saturate(140%)";function z(e){return`color-mix(in srgb, var(--md-sys-color-surface-container, var(--ha-card-background, #f3edf7)) ${Math.round(100*e)}%, transparent)`}const G=i`
  .glass {
    background-color: ${t(z(.72))};
    backdrop-filter: ${t(R)};
    -webkit-backdrop-filter: ${t(R)};
    border: 1px solid
...
`,N=.08,T=.12,W=.12;const j=function(e="currentColor"){const s=t(e);return i`
    .state-layer-target {
      position: relative;
      overflow: hidden;
      isolation: isolate;
  ...
  `}();let A=null,H=0;!function(){if("undefined"==typeof customElements)return;if(H+=1,A)return;A=customElements.define.bind(customElements),customElements.define=(e,t,i)=>{customElements.get(e)||A(e,t,i)}}(),"undefined"!=typeof customElements&&(H>0&&(H-=1),H>0||A&&(customElements.define=A,A=null));let q=class extends l{constructor(){super(...arguments),this.surface="card",this.density="auto",this.defaultView="home",this.previewHeight=200,this._controllerId="",this._masterEntity="",this._pixelCount=210,this._layoutId="",this._fixtureId="",this._previewStatus="connecting",this._hint="",this._globalBriPct=null,this._lastNonZeroBri=100,this._viewReady=!0,this._prevView="home",this._selection=new b(this),this._session=new _(this),this._nav=new w(this,{initial:this.defaultView,normalize:e=>this._normalizeView(e)}),this._bootstrapGen=0,this._bootstrapControllerKey="",this._loadedViewKey="",this._defaultViewSeeded=!1,this._onSegmentTargetsChanged=e=>{this._selection.applyTargetsChanged(e.detail)},this._onSegmentChange=e=>{this._selection.applySegmentChange(e.detail),this._refreshAccent()},this._onPaintConfigChange=()=>{this.requestUpdate()}}get _selectedSegId(){return this._selection.selectedSegId}get _highlightSegIds(){return this._selection.highlightSegIds}get _segments(){return this._selection.segments}get _connection(){return this.connection??this.hass?.connection}get _resolvedDensity(){return"full"===this.density?"full":"compact"}visibleNav(){return k(this.visibleViews,this.surface)}_normalizeView(e){const t=k(this.visibleViews,this.surface);return t.some(t=>t.id===e)?e:t[0]?.id??e}onPoweredConnect(){this._bindConnectionReady(),this._bootstrap()}onPoweredDisconnect(){this._bootstrapGen+=1,this._offConnReady?.(),this._offConnReady=void 0,this._unsubLive?.(),this._unsubLive=void 0}willUpdate(e){super.willUpdate(e),!this._defaultViewSeeded&&e.has("defaultView")&&(this._defaultViewSeeded=!0,this._nav.select(this.defaultView)),(e.has("visibleViews")||e.has("surface")||e.has("defaultView"))&&this._nav.revalidate()}updated(e){if(super.updated(e),e.has("hass")&&null!==this._globalBriPct){const e=this._readGlobalBrightnessPct();(0===e||Math.abs(e-this._globalBriPct)<=1)&&(this._globalBriPct=null)}if((e.has("hass")||e.has("_globalBriPct"))&&this._syncGlobalBriToSlider(),this._ensureActiveViewLoaded(),this._syncPaintWiring(),e.has("controller")||e.has("connection"))return this._bindConnectionReady(),void this._bootstrap(!0);e.has("hass")&&this.hass&&!this._controllerId&&(this._bindConnectionReady(),this._bootstrap())}async _ensureActiveViewLoaded(){const e=this._nav.view;if(this._loadedViewKey!==e)if(this._loadedViewKey=e,B(e))this._viewReady=!0;else{this._viewReady=!1;try{await function(e){const t=E.get(e);if(t)return t;const i=P[e],s=i?i().then(()=>{}):Promise.resolve();return E.set(e,s),s}(e)}finally{this._loadedViewKey===e&&(this._viewReady=!0,this.requestUpdate())}}}_bindConnectionReady(){const e=this._connection;e&&!this._offConnReady&&(this._offConnReady=c(e,()=>{this._bootstrap()}),this.addUnsub(()=>this._offConnReady?.()))}_pickController(e){const t=(this.controller??"").trim();if(!t)return e[0];const i=t.toLowerCase();return e.find(e=>{const s=String(e.title??"");return String(e.entry_id??"")===t||s===t||s.toLowerCase().includes(i)||s.toLowerCase().endsWith(`— ${i}`)||s.toLowerCase().endsWith(`- ${i}`)})??e[0]}_pickLayout(e){const t=(this.layoutId??"").trim();return t?e.find(e=>e.id===t||e.name===t):e[0]}async _bootstrap(e=!1){const t=this._connection;if(!t)return;const i=(this.controller??"").trim();if(!e&&this._controllerId&&this._unsubLive&&this._bootstrapControllerKey===i)return;const s=++this._bootstrapGen;this._controllerId||(this._hint="Connecting to WLED Studio…",this.requestUpdate());const o=[0,400,1200,2500];for(const e of o){if(s!==this._bootstrapGen||!this.isConnected)return;e>0&&await new Promise(t=>setTimeout(t,e));try{const e=await d(t),o=this._pickController(e);if(!o?.entry_id){s===this._bootstrapGen&&(this._hint=0===e.length?"No WLED Studio controllers found. Add the integration under Settings → Devices & services.":"Controller not found in list.",this.requestUpdate());continue}if(s!==this._bootstrapGen)return;return this._controllerId=String(o.entry_id),this._masterEntity=String(o.master_entity_id??""),this._pixelCount=Number(o.pixel_count)||210,this._bootstrapControllerKey=i,this._hint="",await this._loadLayout(),this._startLive(),this._loadSegments(),void this.requestUpdate()}catch(e){const t=e instanceof Error?e.message:String(e??"unknown");s===this._bootstrapGen&&(this._hint=`Connecting… (${t})`,this.requestUpdate())}}s===this._bootstrapGen&&(this._previewStatus="offline",this._preview?.setStatus(this._previewStatus),this._hint="WLED Studio is not responding. In Settings → Devices & services, open WLED Studio — Cloud → Reload, then hard-refresh this page (Ctrl+Shift+R).",this.requestUpdate())}async _loadLayout(){const e=this._connection;if(e&&this._controllerId)try{const t=await h(e,this._controllerId),i=this._pickLayout(t);if(!i)return this._layoutId="",void(this._fixtureId="");this._layoutId=i.id;const s=i.fixtures[0];this._fixtureId=s?String(s.id??"fixture-0"):"fixture-0",i.pixel_count&&(this._pixelCount=i.pixel_count),await(this._preview?.refresh())}catch{this._layoutId="",this._fixtureId=""}}_startLive(){const e=this._connection;if(!e||!this._controllerId)return;const t="live"===this._previewStatus;this._unsubLive?.(),t||(this._previewStatus="connecting",this._preview?.setStatus(this._previewStatus)),this._unsubLive=p(e,this._controllerId,e=>{this._previewStatus="live",this._preview?.setFrame(e)},{remote:this.remote.state.isRemote}),this.addUnsub(()=>this._unsubLive?.())}async _loadSegments(){const e=this._connection;if(e&&this._controllerId)try{const t=(await u(e,this._controllerId)).segments??[];this._selection.setSegments(t),t.length&&this._selection.selectedSegId<0&&this._selection.selectSegment(t[0].id),this._refreshAccent()}catch{}}_onStripSegmentSelect(e){"paint"!==this._nav.view&&(this._selection.selectSegment(e.detail.segmentId),this._refreshAccent())}_syncPaintWiring(){const e=this._nav.view,t=this._prevView;if(this._prevView=e,"paint"===e){const e=this._paintPanel,t=this._preview;return void(e&&t&&this._boundPaintPreview!==e&&(e.bindExternalPreview(t),this._boundPaintPreview=e))}"paint"===t&&this._boundPaintPreview&&(this._boundPaintPreview.cancelLiveIfActive(),this._boundPaintPreview=void 0)}_refreshAccent(){const e=this._selection.segments,t=this._selection.selectedSegId,i=e.find(e=>e.id===t);this._session.applyAccentFromSegment(i)}_readGlobalBrightnessPct(){return this.hass&&this._masterEntity?y(this.hass.states[this._masterEntity]):0}_globalBrightnessPct(){return null!==this._globalBriPct?this._globalBriPct:this._readGlobalBrightnessPct()}_syncGlobalBriToSlider(){const e=this._slider;if(!e)return;const t=this._globalBrightnessPct();var i,s;i={isPowered:this.isPowered},s=()=>{e.value=t},i.isPowered&&s()}_onGlobalBriInput(e){const t=e.target;let i=Number(t.value);0===(this._globalBriPct??this._readGlobalBrightnessPct())&&i>0&&this._lastNonZeroBri>0&&(i=this._lastNonZeroBri,t.value=i),i>0&&(this._lastNonZeroBri=i),this._globalBriPct=i}_setGlobalBrightness(e){if(!this.hass||!this._masterEntity)return;const t=Number(e.target.value);if(0===t){const e=this._globalBriPct??this._readGlobalBrightnessPct();e>0&&(this._lastNonZeroBri=e)}else this._lastNonZeroBri=t;this._globalBriPct=t;const i=$(t);0===t?this.hass.callService("light","turn_off",{entity_id:this._masterEntity}):this.hass.callService("light","turn_on",{entity_id:this._masterEntity,brightness_pct:t});const s=this._connection;s&&this._controllerId&&g(s,this._controllerId,{bri:i,on:t>0})}_togglePower(){this.hass&&this._masterEntity&&this.hass.callService("light","toggle",{entity_id:this._masterEntity})}_onExpand(){const e=this._nav.view,t=this._controllerId||(this.controller??"").trim(),i=new URLSearchParams;i.set("view",e),t&&i.set("controller",t),history.pushState(null,"",`/wled-studio#${i.toString()}`),window.dispatchEvent(new CustomEvent("location-changed"))}_selectView(e){this._nav.select(e)}_onNavActivate(e){const t=e.target,i=t?.activeIndex,s=this.visibleNav();"number"==typeof i&&s[i]&&this._selectView(s[i].id)}_renderHeader(){const t=this.remote.state,i=C(this._nav.view)??I[0],s=this.controller?.trim()||i.label||"WLED Studio";return e`
    ...
    `}_renderPreview(){const t="paint"===this._nav.view,i=`--wled-preview-height: ${this.previewHeight}px`,s=this._paintPanel,o=s?.brushSize??6,n=!!t&&(s?.paintLivePreview??!1);return e`
      <wled-geometry-preview
        class="preview"
        style=${i}
        compact
        .externalLive=${!t||(s?.paintExternalLive??!0)}
        .paintLivePreview=${n}
        .paintBrushSize=${o}
    ...
    `}_renderBar(){const t=this.visibleNav(),i=this._nav.view,s=Math.max(0,t.findIndex(e=>e.id===i));return e`
      <md-navigation-bar
        class="nav nav-bar"
        part="nav"
        .activeIndex=${s}
    ...
    `}_viewContext(){const e=this._resolvedDensity;return{hass:this.hass,connection:this._connection,controllerId:this._controllerId,masterEntity:this._masterEntity,layoutId:this._layoutId,fixtureId:this._fixtureId,pixelCount:this._pixelCount,selectedSegId:this._selectedSegId,highlightSegIds:this._highlightSegIds,segments:this._segments,density:e,compact:"compact"===e,onSegmentChange:this._onSegmentChange,onSegmentTargetsChanged:this._onSegmentTargetsChanged,onPaintConfigChange:this._onPaintConfigChange}}_renderBody(){const t=this._nav.view;return B(t)||this._viewReady?e`<div class="body" part="body">
      ${function(e,t){return L[e](t)}(t,this._viewContext())}
    ...
    `]}};s([o({reflect:!0})],q.prototype,"surface",void 0),s([o({reflect:!0})],q.prototype,"density",void 0),s([o({reflect:!0,attribute:"default-view"})],q.prototype,"defaultView",void 0),s([o({attribute:!1})],q.prototype,"visibleViews",void 0),s([o({attribute:!1})],q.prototype,"controller",void 0),s([o({attribute:!1})],q.prototype,"layoutId",void 0),s([o({attribute:!1})],q.prototype,"previewHeight",void 0),s([o({attribute:!1})],q.prototype,"connection",void 0),s([n()],q.prototype,"_controllerId",void 0),s([n()],q.prototype,"_masterEntity",void 0),s([n()],q.prototype,"_pixelCount",void 0),s([n()],q.prototype,"_layoutId",void 0),s([n()],q.prototype,"_fixtureId",void 0),s([n()],q.prototype,"_previewStatus",void 0),s([n()],q.prototype,"_hint",void 0),s([n()],q.prototype,"_globalBriPct",void 0),s([n()],q.prototype,"_lastNonZeroBri",void 0),s([n()],q.prototype,"_viewReady",void 0),s([a("wled-geometry-preview")],q.prototype,"_preview",void 0),s([a("md-slider")],q.prototype,"_slider",void 0),s([a("wled-view-paint")],q.prototype,"_paintPanel",void 0),q=s([m("wled-studio-shell")],q);const U="wled-studio-card",K=["color","effects","scenes","segments","paint"];class F extends l{setConfig(e){if(!e.type?.startsWith("custom:"))throw new Error("Invalid card type");this.config=e}getCardSize(){return 8}getGridOptions(){return{columns:12,min_columns:6}}static getConfigElement(){const e=document.createElement("wled-studio-card-editor");return e.setConfig(F.getStubConfig()),e}static getStubConfig(){return{type:`custom:${U}`,controller:"Cloud",height:200,show_segments:!1}}_resolveVisibleViews(){const e=this.config;if(e)return e.views&&e.views.length?[...e.views]:K.filter(t=>("scenes"!==t||!1!==e.show_scenes)&&(("paint"!==t||!1!==e.show_paint)&&(("effects"!==t||!1!==e.show_effects)&&("segments"!==t||!0===e.show_segments))))}_resolveDefaultView(){return this.config?.default_view??I[0].id}render(){const t=this.config,i=t?.height??200,s=t?.density??"auto",o=this._resolveVisibleViews(),n=this._resolveDefaultView();return e`
      <div class="card-root">
        ${x()?e`
    ...
    `]}}function M(){return{type:`custom:${U}`,controller:"",height:200,show_segments:!1}}s([o({attribute:!1})],F.prototype,"config",void 0);const Z=[{key:"show_effects",label:"Show Effects tab (legacy)"},{key:"show_scenes",label:"Show Scenes tab (legacy)"},{key:"show_segments",label:"Show Segments tab (legacy)"},{key:"show_paint",label:"Show Paint tab (legacy)"}],O=[{value:"auto",label:"Auto (container decides)"},{value:"compact",label:"Compact (bottom bar)"},{value:"full",label:"Full (left rail)"}],J=I.filter(e=>"full"!==e.density),Q=I.filter(e=>"full"===e.density),X=new Set(J.map(e=>e.id));let Y=class extends r{constructor(){super(...arguments),this._config=M()}setConfig(e){this._config={...M(),...e,type:e.type??`custom:${U}`}}render(){const t=this._config,i=t.views??[];return e`
      <div class="editor">
    ...
    `}_onController(e){this._fire({...this._config,controller:e.detail.value})}_onHeight(e){const t=Number(e.detail.value);this._fire({...this._config,height:Number.isFinite(t)?t:200})}_onLayoutId(e){const t=e.detail.value.trim(),i={...this._config};t?i.layout_id=t:delete i.layout_id,this._fire(i)}_onDensity(e){const t=e.target.value,i={...this._config};"auto"===t?delete i.density:i.density=t,this._fire(i)}_onDefaultView(e){const t=e.target.value,i={...this._config};t&&X.has(t)?i.default_view=t:delete i.default_view,this._fire(i)}_onViewToggle(e,t){if(!X.has(e))return;const i=t.target.checked,s=this._config.views??[];let o;o=i?s.includes(e)?s:[...s,e]:s.filter(t=>t!==e);const n={...this._config};o.length?n.views=o:delete n.views,this._fire(n)}_onTabToggle(e,t){const i=t.target.checked;this._fire({...this._config,[e]:i})}_fire(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0}))}static{this.styles=i`
    .editor {
      display: flex;
      flex-direction: column;
      gap: 12px;
  ...
  `}};s([o({attribute:!1})],Y.prototype,"hass",void 0),s([n()],Y.prototype,"_config",void 0),Y=s([m("wled-studio-card-editor")],Y),S(),f(U,F),window.customCards=window.customCards||[],window.customCards.some(e=>e.type===U)||window.customCards.push({type:U,name:"WLED Studio",description:"Live LED strip preview and controls",preview:!0}),console.info("[wled-studio] lovelace bundle loaded",{card:U});
